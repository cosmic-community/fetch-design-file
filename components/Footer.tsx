  ...
  return (
    <footer className="bg-amber-950 text-white">
      <div className="max-w-7xl mx-auto px-6 pt-16 pb-8">
        {/* Changed: multi-column footer layout */}
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 mb-12">
          ...
          <div className="col-span-2">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="font-display font-bold text-base">X Marks the Spot</span>
            </div>
            <p className="text-sm text-amber-300/50 max-w-xs leading-relaxed mb-6">{tagline}</p>
            {/* Social icons */}
            <div className="flex gap-3">
              {['twitter', 'github', 'linkedin'].map((social) => (
                <a key={social} href="#" className="w-8 h-8 rounded-lg bg-amber-900/50 flex items-center justify-center text-amber-400/60 hover:text-amber-300 hover:bg-amber-900 transition-colors">
          ...
          {Object.entries(footerLinks).map(([heading, links]) => (
            <div key={heading}>
              <h4 className="font-semibold text-sm text-amber-100/80 mb-4">{heading}</h4>
              <ul className="space-y-2.5">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-sm text-amber-400/50 hover:text-amber-200 transition-colors">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="pt-8 border-t border-amber-900/50 flex flex-col sm:flex-row items-center justify-between gap-4">
